...
const buildOptions = () => {
  ...
  fs.writeFileSync('./lib/get-options.js', [
    '"use strict";',
...
const printMissingOptions = (includedOptions) => {
  var dateRangeOptions = fs.readFileSync(`${__dirname}/../node_modules/bootstrap-daterangepicker/daterangepicker.js`, 'utf-8')
			.toString().split(' ').filter((item) => {
				return item.indexOf('options.') === 0;
			}).map((item) => {
				return item.split('.')[1].split('[')[0].replace(/[^a-zA-Z0-9\<\>]/gi, '');
			});
	const missingOptions = dateRangeOptions.filter((item, index) => {
